...
//Set state.query when the user types a letter
  const queryChange = (event)=>{
    ...
    setState(state=>{return {...state,query:newQuery}})
  }
...
//When there is only one country to show, update state.city for showing weather
  useEffect(()=>{
    if(state.list.length === 1){
      setState(state=>{return {...state,city:state.list[0].capital+', '+state.list[0].alpha2Code}})
    } else if (state.list.length > 1 && state.city !== ''){
      setState(state=>{return {...state,city:''}})
    }
  },[state.list])

  //Retrieve weather city is set, round temps and wind speed, convert wind degree to direction
  useEffect(()=>{
    if(state.city !== ''){
